$(document).ready(function() {
	...
	var app = {
		...
		route: function() {
			...
			if (!hash) {
				$('body').html(new StartView(this.store).render().el);
				return;
			}

			// Switch tussen pages
			var page = hash.match(app.anotherPageURL);
			// Is er een page die moet worden opgeroepen?
			if (page) {
				$('body').html(new AnotherView(this.store).render().el);
				return;
			}
		},
		...
		initialize: function() {
			...
			this.anotherPageURL = '#another-page';
			this.registerEvents();
